<template>
  ...
  <div class="panel">
    ...
    <el-table :data="data.items" border v-loading="loading" row-key="code">
      ...
      <el-table-column label="操作" min-width="285" fixed="right"><template #default="{ row }"><el-button link :type="row.status === 'running' ? 'warning' : 'success'" :loading="acting === row.code" @click="toggle(row)">{{ row.status === 'running' ? '关闭' : '打开' }}</el-button><el-button link type="primary" @click="drawer?.edit(row.code)">编辑</el-button><el-button link @click="clearCache(row.code)">清理缓存</el-button><el-button link type="danger" @click="remove(row.code)">删除</el-button></template></el-table-column>
    </el-table>
