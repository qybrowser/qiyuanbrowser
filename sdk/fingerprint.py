...
def generate_user_agent(platform: str, version: str, kernel: str = "chrome") -> str:
    ...
    if kernel == "firefox":
        ...
        return f"Mozilla/5.0 ({os_part}; rv:{major}.0) Gecko/20100101 Firefox/{major}.0"
...
def _random_hex64() -> str:
    return "".join(random.choice("0123456789abcdef") for _ in range(64))


def _random_media_devices(platform: str) -> List[Dict[str, str]]:
    mac = platform.lower() in ("macintel", "macintosh", "darwin")
    microphones = ["MacBook Pro Microphone", "Built-in Microphone"] if mac else [
        "麦克风阵列 (Realtek(R) Audio)", "麦克风阵列 (适用于数字麦克风的英特尔® 智音技术)"
    ]
    cameras = ["FaceTime HD Camera", "FaceTime HD Camera (Built-in)"] if mac else [
        "Integrated Camera", "USB2.0 HD UVC WebCam"
    ]
    speakers = ["MacBook Pro Speakers", "Built-in Output"] if mac else [
        "扬声器 (Realtek(R) Audio)", "Speakers (Realtek High Definition Audio)"
    ]
    return [
        {"deviceId": _random_hex64(), "kind": "audioinput", "label": random.choice(microphones), "groupId": _random_hex64()},
        {"deviceId": _random_hex64(), "kind": "videoinput", "label": random.choice(cameras), "groupId": _random_hex64()},
        {"deviceId": _random_hex64(), "kind": "audiooutput", "label": random.choice(speakers), "groupId": _random_hex64()},
    ]
...
def generate_fingerprint(platform: str = "Win32") -> Dict[str, object]:
    ...
    return {
        ...
        "media_devices": _random_media_devices(platform),
...
def build_fingerprint_payload(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "media_devices" in data:
        out["media_devices"] = None if data["media_devices"] == "disabled" else _random_media_devices(str(data.get("platform") or "Win32"))
...
def build_fingerprint_json(
    ...
    payload = {
        ...
        "native_fp_protection": 1 if fp.get("native_fp_protection") else 0,
